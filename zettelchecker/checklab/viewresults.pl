...
my $courseroot = "/home.local/vogt/checklabresults/data";
...
# Get the Name of the Student:
my $sheetnumber = $cgi->param('sheetnumber');
# Get the PIN of the Student:
my $examinerpassword = $cgi->param('examinerpassword');
...
print "<html><head><title>$coursename: Tasksheet $sheetnumber - ISIP Task Submission</title></head><body bgcolor='#E0E0E0'>\n";
print "<h1>$coursename: Tasksheet $sheetnumber</h1>\n";

# Check if the PIN is correct...
my $pincorrect = "false";
# Really basic password control. May be exposed by just turning off Perl on the webserver?
if ($examinerpassword eq "blalala")
{
	$pincorrect = "true";
}
...
{
	print "Sorry, but the password you entered is incorrect!<br>\n";
	$continue = "false";
}
else
{
